...
const CancelarInvitacion = ({
    ...
    const handleCancel = async () => {
        try {
            ...
            setLoading(false)
            setMessage('Se ha cancelado la invitacion')
            setVisibleOk(true)
        } catch (error) {
            console.error(error);
        ...
        <>
            <Portal>
                <Dialog visible={visible} onDismiss={() => setVisible(false)} style={{ backgroundColor: theme.colors.primary }}>
                    ...
                    <Dialog.Title style={[styleDialog.title, { color: theme.colors.white }]}>¿Desea cancelar la invitacion?</Dialog.Title>
                    <Dialog.Actions style={styleDialog.dialogActions}>
                        ...
                        <Icon.Button name={theme.icons.ok} backgroundColor={theme.colors.white} color={theme.colors.primary} onPress={handleCancel}>Aprobar
                        </Icon.Button>
                    </Dialog.Actions>
                </Dialog>
            </Portal>
...
export default CancelarInvitacion